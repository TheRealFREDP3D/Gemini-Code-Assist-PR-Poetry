...
This script fetches and saves the complete conversation history from GitHub pull requests,
including PR details, issue comments, review comments, and reviews.

The script uses a .env file to get the GitHub token. Create a .env file with:
...
repo_owner = "TheRealFREDP3D"  # Default repository owner
repo_name = "Gemini-Code-Assist-PR-Poetry"  # Default repository name
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Fetch GitHub pull request conversations")
    parser.add_argument(
        "--token",
